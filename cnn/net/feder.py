...
class SED(nn.Module):
    def __init__(self, in_channels, mid_channels):
        ...
        self.reu=REU6(in_channels, mid_channels)
        self.reduce1 = Conv1x1(256, 16)
        self.reduce2 = Conv1x1(512, 16)
        self.reduce3 = Conv1x1(1024, 16)
        self.reduce4 = Conv1x1(2048, 16)
        self.reduce5 = Conv1x1(256, 1)
    ...
    def forward(self, x4, x3, x2, x1):
        ...
        x11=self.reduce5(x11)
        obj,eg=self.reu(out, x11)
        eg = F.interpolate(eg, size, mode='bilinear', align_corners=False)
        return eg
...
if __name__ == '__main__':
    f1 = torch.randn(16, 1, 136, 136).cuda()
    hh = torch.randn(16, 64, 136, 136).cuda()
    hh1 = torch.randn(16, 256, 136, 136).cuda()
    hh2 = torch.randn(16, 512, 136, 136).cuda()
    hh3 = torch.randn(16, 1024, 136, 136).cuda()
    hh4= torch.randn(16, 2048, 136, 136).cuda()

    r6=REU6(64,64).cuda()
    o,b=r6(hh,f1)
    print(b.shape)
    sed=SED(64,64).cuda()
    eg=sed(hh4,hh3,hh2,hh1)
    print(eg.shape)
