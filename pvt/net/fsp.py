...
class Model(nn.Module):
    def __init__(self, img_size=384):
        ...
        self.reduce1 = Conv1x1(64, 192)
        self.reduce2 = Conv1x1(128, 192)
        self.reduce3 = Conv1x1(320, 192)
        self.reduce4 = Conv1x1(512, 192)
        self.reduce51 = Conv1x1(192, 1)
        self.reduce52 = Conv1x1(192, 1)
        self.reduce53 = Conv1x1(384, 1)
        self.reduce54 = Conv1x1(768, 1)
        self.img_size = img_size
        self.vit_chs = 768
    ...
    def forward(self, x4, x3, x2, x1):
        ...
        x3 = self.reduce3(x3)
        x4 = self.reduce4(x4)
        x4 = F.interpolate(x4, (24,24), mode='bilinear', align_corners=False)
        x3 = F.interpolate(x3, (24,24), mode='bilinear', align_corners=False)
        x2 = F.interpolate(x2, (24,24), mode='bilinear', align_corners=False)
        x1 = F.interpolate(x1, (24,24), mode='bilinear', align_corners=False)

        out = torch.concat((x4,x3,x2, x1), dim=1)
        arr = []
        for i in range(12):
            arr.append(out)
        out4,out3,out2,out1 = self.group_pyramid_decode(arr)
        out1=self.reduce51(out1)
        out2=self.reduce52(out2)
        out3=self.reduce53(out3)
        out4=self.reduce54(out4)
        out1 = F.interpolate(out1, (704, 704), mode='bilinear', align_corners=False)
        out2 = F.interpolate(out2, (704, 704), mode='bilinear', align_corners=False)
        out3 = F.interpolate(out3, (704, 704), mode='bilinear', align_corners=False)
        out4 = F.interpolate(out4, (704, 704), mode='bilinear', align_corners=False)
        return out4, out3, out2, out1
if __name__ == '__main__':


# [8, 64, 176, 176])
# torch.Size([8, 128, 88, 88])
# torch.Size([8, 320, 44, 44])
# torch.Size([8, 512, 22, 22])
    hh1 = torch.randn(8, 64, 176, 176).cuda()
    hh2 = torch.randn(8, 128, 88, 88).cuda()
    hh3 = torch.randn(8, 320, 44, 44).cuda()
    hh4= torch.randn(8, 512, 22, 22).cuda()

    m=Model().cuda()
    out=m(hh4,hh3,hh2,hh1)
    print(out)
